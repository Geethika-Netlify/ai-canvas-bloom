...
export function Navbar() {
  ...
  const handleItemClick = (label: string) => {
    setActiveItem(label);
    const element = document.querySelector(menuItems.find(item => item.label === label)?.href || "");
    element?.scrollIntoView({
      behavior: "smooth"
    });
  };
  return <header className="bg-transparent">
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-center">
        <div className="flex items-center justify-center gap-4">
          <MenuBar items={menuItems} activeItem={activeItem} onItemClick={handleItemClick} className="bg-transparent border-none shadow-none" />
          <ThemeToggle />
        </div>
      </div>
    </header>;
